...
;; Defensive guard against underflow (stacks-block-height < payment block)
(define-read-only (is-payment-fresh (nonce (buff 16)))
  (match (map-get? payments { nonce: nonce })
    payment (if (>= stacks-block-height (get block payment))
              (< (- stacks-block-height (get block payment)) PAYMENT-LIFETIME)
              false)
    false
  )
)
