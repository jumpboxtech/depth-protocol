...
;; Defaults to none - must be set post-deploy before completions can be recorded
(define-data-var task-board-contract (optional principal) none)
...
(define-public (record-completion
  ...
  (begin
    (asserts! (is-eq (some contract-caller) (var-get task-board-contract)) ERR-NOT-AUTHORIZED)
...
(define-public (set-task-board (contract principal))
  (begin
    ...
    (var-set task-board-contract (some contract))
...
(define-public (accept-admin)
  (let
    ...
    (asserts! (is-eq contract-caller pending) ERR-NOT-AUTHORIZED)
...
(define-read-only (is-task-board-set)
  (is-some (var-get task-board-contract))
)
