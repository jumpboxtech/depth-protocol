...
;; Deregistering decrements total-agents (only once per agent)
(define-public (set-status (new-status uint))
  (let
    (
      (agent (unwrap! (map-get? agents { owner: tx-sender }) ERR-NOT-REGISTERED))
      (was-active (or (is-eq (get status agent) STATUS-ACTIVE) (is-eq (get status agent) STATUS-PAUSED)))
      (going-deregistered (is-eq new-status STATUS-DEREGISTERED))
    )
    ...
    (asserts! (or (is-eq new-status STATUS-ACTIVE)
              ...
              ERR-INVALID-STATUS)
    (asserts! (not (is-eq (get status agent) STATUS-DEREGISTERED)) ERR-INVALID-STATUS)

    (map-set agents { owner: tx-sender } (merge agent { status: new-status }))

    ;; Decrement total-agents when transitioning to deregistered
    (if (and was-active going-deregistered)
      (var-set total-agents (- (var-get total-agents) u1))
      true
    )
...
(define-public (record-task-completion (agent principal) (earned uint))
  (let
    ...
    ;; Only the authorized task-board contract can call this
    (asserts! (is-eq contract-caller (var-get task-board-contract)) ERR-UNAUTHORIZED)
...
;; Set the authorized task-board contract (must be called post-deploy)
