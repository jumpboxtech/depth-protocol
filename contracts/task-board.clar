...
;;              Escrow held by contract via as-contract.
...
(define-public (approve (task-id uint))
  (let
    ...
    (try! (as-contract? ((with-stx bounty))
...
(define-public (dispute (task-id uint) (reason-url (string-utf8 256)))
  (let
    ...
    ;; Must be submitted
    (asserts! (is-eq (get status task) TASK-SUBMITTED) ERR-INVALID-STATUS)
    ;; Within dispute window (strictly less than - window is exclusive)
...
(define-public (resolve-dispute
  ...
  (let
    ...
    (if (> pay-agent u0)
      (try! (as-contract? ((with-stx pay-agent))
    ...
    (if (> refund-poster u0)
      (try! (as-contract? ((with-stx refund-poster))
...
(define-public (cancel (task-id uint))
  (let
    ...
    (try! (as-contract? ((with-stx total-escrow))
